...
| 目的 | リポジトリ | 参考にする部分 |
|---|---|---|
| 設計全体 | Market-Data-Dashboard | yfinance+FREDの統合構造 |
| 相関計算 | QuantStats | Rolling Correlationの実装 |
| レジーム判定（将来） | MarketRegimeDetection | HMMロジック（初版後に導入検討） |
| 相関パターン | IntermarketAnalysis | 債券↔株↔暗号資産の関係分類 |
| パイプライン設計 | Finance-Data-Pipeline | 複数API→DB保存の構造 |
| センチメント | Crypto-Fear-Greed-Index-Trend-Analysis | F&Gスコアの売買シグナル変換 |
