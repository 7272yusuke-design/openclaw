...
| **neo-acp-seller.service** | ✅ systemd管理下で稼働中（4 offerings提供中） |
...
| **Moltbook** | **neoautonomous** karma=1・followers=3・posts=4（旧Oracle_of_Order完全排除済み） |
...
| **Neo自身のGraduation** | テストBuyer作成→Neoに10件発注→動画デモ→申請 | — | USDC数ドル（$0.30×10=$3程度・ガス代スポンサー済み） |
| **テストBuyer作成** |  → wallet取得 →  → Seller再起動 | — | Seller停止数分のみ |
...
| **Buyer戦略: 情報購入→Council統合** | Orion/Wolfpack等の情報offeringをBuyer発注→Phase 1-Aに注入→勝率改善検証 | 3h | Graduation後Phase 2の収益基盤 |
...
### v6.5t（2026/03/31 — N.1 Z-scoreスコアリング統合・旧アカウント完全排除・戦略調査）

| Task | 内容 | 結果 |
|---|---|---|
| **moltbook_tracker AGENT_NAME修正** | Oracle_of_Order → neoautonomous | 完了 ✅ |
| **moltbook_stats.jsonリセット** | 旧アカウントデータ削除・新アカウントで初期化 | 完了 ✅ |
| **N.1 Z-score Phase 4b統合** | _pt_zデフォルト初期化 + Z±1.0/±1.5で±4/±8点スコアリング + ログ表示 | 完了 ✅ |
| **DEXアービトラージ収益性分析** | 2%スプレッドの正体はWETH/USDCペア間差と判明。同一ペア内0.17%で手数料負け→収益化不可 | 調査完了 |
| **Graduation要件調査** | Seller側10件成功（自前テストBuyerでOK・3件連続成功必要）・ガス代スポンサー済み | 調査完了 |
| **ACP経済圏構造分析** | Provider過多・Buyer希少。情報層→実行層パイプライン上にBuyerとして参入する戦略を策定 | 調査完了 |

### v6.5s（2026/03/30 — Moltbookエンゲージャー全面刷新・新アカウント）

| Task | 内容 | 結果 |
|---|---|---|
| **myposts CLIバグ発見・修正** | getPosts()が全体フィード返却→自分の投稿取得不能 | moltbook_wrapper.mjs修正 ✅ |
| **新Moltbookアカウント** | neoautonomous登録・クレーム完了（旧Oracle_of_Orderは廃止） | 稼働中 ✅ |
| **エンゲージャーREST API化** | check_and_reply_comments → /home APIベース | 完了 ✅ |
| **engage_feed REST API化** | CLI依存排除 + verification challenge対応 | 完了 ✅ |
| **セマンティック検索営業** | search_and_engage新メソッド（/search APIで営業コメント） | 完了 ✅ |
| **moltbook_tool.py REST API化** | post()をREST API直接呼び出し + verification対応 | テスト投稿成功 ✅ |
| **get_engagement_stats REST API化** | CLI依存排除・動的agent名判定 | 完了 ✅ |
...
Phase 1: Graduation達成（直近）
  ├ テストBuyer作成 → Neoに10件発注（$3程度・ガス代スポンサー済み）
  ├ 動画デモ録画 → Graduation申請 → DevRel審査（~7営業日）
  └ Graduation → Butler検索露出 → 集客自動化
     ⬇
Phase 2: サービス拡充 + Buyer戦略
  ├ 取引系offerings再公開（実績を土台に）
  ├ Buyer戦略: Orion/Wolfpack情報購入→Council精度向上→実績証明
  ├ オーケストレーター型offering（複数Provider統合+品質評価）
  ├ 学習100回完了→通常モード
  └ 情報層パイプライン参入（Axelrod/Otto等の実行層と連携）
     ⬇
Phase 3: 本格稼働
