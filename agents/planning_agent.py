...
class PlanningCrew(NeoBaseCrew):
    ...
    def run(self, goal: str, context: str, sentiment_score: float = 0.0, market_trends: str = ""):
        ...
        auditor = Agent(
            ...
            llm=NeoConfig.get_agent_llm(NeoConfig.REASONING_MODEL), # Agent LLM (OpenRouter)
            max_iter=NeoConfig.MAX_ITER,
            allow_delegation=False
        )
        ...
        crew = Crew(
            ...
            manager_llm=NeoConfig.get_agent_llm(NeoConfig.MODEL_BRAIN), # Agent Manager (OpenRouter)
            **common_params
        )
