...
- **Knowledge Sharing:** **Shared Blackboard** (vault/blackboard/live_intel.json) による非同期連携
...
## 🧠 Memory Strategy (The Iron Rule of Double Recording)
**Rule: Always record progress in TWO places at the end of a session.**

### 1. Daily Log (`memory/YYYY-MM-DD.md`) - The Voyage Log
- **When:** Update after every major task or at session end.
- **What:** Detailed execution logs, command outputs, error traces, reasoning steps, and specific TODOs.
- **Purpose:** Full traceability. "What exactly did we do today?"

### 2. Long-Term Memory (`MEMORY.md`) - The History Book
- **When:** Update at session end.
- **What:** Key decisions, architectural changes, lessons learned, competitor analysis, and current project status.
- **Purpose:** Context retention. "Who am I now, and what do I know?"

### 3. Automated Logging
- **`logs/market_cycle.jsonl`:** Raw operational data (prices, sentiment scores) generated by background scripts. Use this for quantitative analysis.
